...
class Go2Env:
    ...
    def _resample_commands(self, envs_idx):
        """Sample commands from curriculum-adjusted ranges, with standing envs."""
        n = len(envs_idx)
        if n == 0:
            return
        ...
        # Standing envs: zero commands for bottom N% of env indices
        if self._standing_env_indices is not None and len(self._standing_env_indices) > 0:
            # Find intersection of envs_idx and standing indices
            standing_mask = torch.isin(envs_idx, self._standing_env_indices)
            if standing_mask.any():
                standing_in_batch = envs_idx[standing_mask]
                self.commands[standing_in_batch, :] = 0.0
